...
-- 1. Név szerint rendezve jelenjenek meg a 10. C-s diákok.
SELECT nev
FROM diak
WHERE osztaly = 'C' AND evfolyam = 10
ORDER BY nev;

-- 2. Listázd azon tanárok nevét és szakkörét, amelyek tartalmazzák a  "programoz" vagy "robotika" szövegrészeket.
SELECT tanar_nev, szakkor
FROM szakkor
WHERE szakkor LIKE '%programoz%' OR szakkor LIKE '%robotika%';

-- 3. Jelenítsd meg azon szakkörök nevét, az azt tartó tanárt és az azokra jelentkező diákok számát, ahol legalább 10-en jelentkeztek.

SELECT szakkor, tanar_nev, COUNT(d.id) AS diak_szam
FROM szakkor
JOIN diak d ON d.szakkor_id = szakkor.id
GROUP BY szakkor, tanar_nev
HAVING COUNT(d.id) >= 10;

/*

	4. Jelenjenek meg azon osztályok (év, betű), amelyeket a Matematika 
	munkaközösség hirdetett meg. 
	- egy osztály csak egyszer szerepeljen
	- rendezés évfolyam, aztán betűjel szerint

*/

SELECT DISTINCT d.evfolyam, d.betujel
FROM diak d
JOIN szakkor s ON s.id = d.szakkor_id
WHERE s.munkakozossag = 'Matematika'
ORDER BY d.evfolyam, d.betujel;

/* 

	5. Mit tesz az alábbi lekérdezés:
...
SELECT diak1.nev, szakkor.nev 
FROM diak AS diak1
JOIN jelentkezes AS jelentkezes1 ON diak1.azon = jelentkezes1.diakazon
JOIN szakkor ON jelentkezes1.szakazon = szakkor.azon
JOIN diak AS diak2 ON diak2.evfolyam = diak1.evfolyam
JOIN jelentkezes AS jelentkezes2 ON diak2.azon = jelentkezes2.diakazon AND jelentkezes2.szakazon = szakkor.azon
WHERE diak2.nev = "Beke Fanni"
	AND diak1.nev <> "Beke Fanni";

/*

	6. Jelenjenek meg a "2. idegen nyelv" munkaközösség 
	által hirdetett szakkörök. Kérdezd le a kapcsolódó tanárok 
	és diákok nevét és osztályát.

 */

SELECT szakkor.szakkor AS szakkor_nev, tanar.tanar_nev, diak.nev AS diak_nev, diak.osztaly
FROM szakkor
JOIN tanar ON szakkor.tanar_id = tanar.id
JOIN diak ON szakkor.id = diak.szakkor_id
WHERE szakkor.munkakozossag = '2. idegen nyelv';
